...
platform.add_source("gpio.v") #Javier

# Create our soc (fpga description)
class BaseSoC(SoCCore):
    def __init__(self, platform):
        sys_clk_freq = int(125e6)
        interrupt_map={    #Javier
            'Soc_gpio': 7,
        }
        ...
        self.submodules.Soc_gpio = gpio() #Creacion del perferico
        self.add_csr("Soc_gpio")
        self.comb += [
            self.Soc_gpio.CLK.eq(ClockSignal()), #Entrada
            self.Soc_gpio.RST.eq(ResetSignal()), #Entrada
            self.Soc_gpio.SW.eq(gpio_sw),        #Entrada  
	        gpio_leds.eq(self.Soc_gpio.LED)]     #Salida
...
builder.build(build_name="soc_Javier")
